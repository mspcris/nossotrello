...
(function () {
  ...
  const daySel  = ".cm-cal-day[data-day], .cm-week-col[data-day]"; // month + week
  ...
  function cardsBox(dayEl) {
    ...
    return (
      dayEl.querySelector?.(".cm-cal-cards") ||
      dayEl.querySelector?.(".cm-week-cards") ||
      dayEl
    );
  }
  ...
  function enableCard(card) {
    ...
    card.addEventListener("dragstart", (e) => {
      draggingCard = card;
      originDay = card.closest(daySel);

      card.classList.add("is-dragging");

      // Firefox exige setData
  ...
  // Bind draggable nos cards (continua igual)
  root.querySelectorAll(cardSel).forEach(enableCard);

  // Event delegation (drop/dragover) no root: funciona mês + semana
  if (root.dataset.cmDragDelegated === "1") return;
  root.dataset.cmDragDelegated = "1";

  root.addEventListener("dragover", (e) => {
    const day = e.target.closest(daySel);
    if (!day) return;

    // precisa para permitir drop
    e.preventDefault();
  ...
  root.addEventListener("drop", async (e) => {
    ...
    const newDate = day.dataset.day;
    const cardId = draggingCard.dataset.cardId;
    const field = getActiveField();
    if (!newDate || !cardId) return;

    const from = originDay.dataset.day;
    if (from === newDate) return; // sem alteração

    // UI otimista
    const toBox = cardsBox(day);
    if (toBox) toBox.appendChild(draggingCard);
