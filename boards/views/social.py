...
def social_cover_upload(request):
    ...
    if f:
        ...
        # Mantém o histórico completo de capas (sem dedupe): cada upload
        # vira um post novo que aparece no perfil e no reel "Novidades".
        SocialPost.objects.create(
            user=request.user,
...
def social_avatar_upload(request):
    ...
    # Auto-post usa a mesma imagem comprimida — mantém o histórico
    # completo de fotos de perfil (aparece tanto no perfil quanto no reel
    # "Novidades"). Não dedupe os anteriores: o usuário quer ver o legado.
    SocialPost.objects.create(
        user=request.user,
...
# Procura por nome, handle e email; inclui amigos, convites pendentes
# e sugestões, agrupados na resposta.
# ---------------------------------------------------------------
@login_required
def social_user_search(request):
    q = (request.GET.get("q") or "").strip()
    if len(q) < 2:
        return JsonResponse({"friends": [], "pending": [], "suggestions": []})
    ...
    users = list(
        ...
        .order_by("profile__display_name")[:30]
    )

    # Classifica cada match em uma das três faixas.
    accepted_ids = set(_accepted_friend_ids(me))
    pending_sent = set(
        SocialFriendship.objects.filter(requester=me, status="pending")
        .values_list("receiver_id", flat=True)
    )
    pending_received = set(
        SocialFriendship.objects.filter(receiver=me, status="pending")
        .values_list("requester_id", flat=True)
    )
    ...
    friends, pending, suggestions = [], [], []
    for u in users:
        c = _card(u)
        if u.id in accepted_ids:
            friends.append(c)
        elif u.id in pending_sent:
            c["pending_direction"] = "sent"
            pending.append(c)
        elif u.id in pending_received:
            c["pending_direction"] = "received"
            pending.append(c)
        else:
...
def _health_food_context(user):
    ...
    food_keywords = [
        ...
        "arroz", "feijão", "feijao", "salada", "fruta",
        "suco", "água", "agua", "proteína", "proteina",
        "carboidrato", "verdura", "legume", "carne", "frango",
        "peixe", "ovo", "leite", "pão", "pao", "macarrão",
        "macarrao", "pizza", "hamburguer", "salgado", "doce",
...
# ---------------------------------------------------------------
# Saúde e Bem Estar — chat contínuo
# ---------------------------------------------------------------
@login_required
@require_POST
