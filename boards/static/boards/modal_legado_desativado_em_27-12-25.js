...
// =====================================================
// Breadcrumb do Modal: contexto dinâmico (Meus quadros, Compartilhados, Favoritos…)
// =====================================================
(function () {
  function qs(sel, root) { return (root || document).querySelector(sel); }

  function getBoardsIndexUrl() {
    const logo = qs('header a[href]:not(#open-user-settings)');
    return logo ? logo.getAttribute("href") : "/";
  }

  function getContextLabel() {
    // tenta capturar como o quadro aparece na home (ex: "Meus quadros" ou "Compartilhados comigo")
    const currentBoardName = (document.title || "").trim().toLowerCase();
    const cards = Array.from(document.querySelectorAll(".board-card, .nt-link, a"));
    for (const c of cards) {
      const text = (c.textContent || "").trim().toLowerCase();
      if (currentBoardName && text.includes(currentBoardName)) {
        const parentSection = c.closest("section, div");
        if (parentSection) {
          const title = parentSection.querySelector("h2, h3, .section-title");
  ...
  function getBoardName() {
    const byId = qs("#board-title");
    const byTitle = (document.title || "").trim();
    const byH1 = qs("h1");
    const raw = (byId && byId.textContent) || byTitle || (byH1 && byH1.textContent) || "";
  ...
  function ensureBreadcrumbEl() {
    const dotsBar = qs("#card-modal-root .theme-dot")?.closest("div") || qs("#card-modal-root");
    if (!dotsBar) return null;
    let el = qs("#modal-breadcrumb");
    if (el) return el;
    el = document.createElement("div");
    el.id = "modal-breadcrumb";
    el.className = "mr-3 text-sm font-medium whitespace-nowrap overflow-hidden text-ellipsis";
    dotsBar.parentNode.insertBefore(el, dotsBar);
    return el;
  }
  ...
  function renderBreadcrumb() {
    const host = ensureBreadcrumbEl();
    if (!host) return;
    const boardsUrl = getBoardsIndexUrl();
    const contextLabel = getContextLabel();
    const boardName = getBoardName();
    const cardTitle = getCardTitle();
    const sep = " <span class='opacity-60'>→</span> ";
    host.innerHTML =
      `<a href="${boardsUrl}" class="underline underline-offset-2">${escapeHtml(contextLabel)}</a>` +
      sep +
      `<span>${escapeHtml(boardName)}</span>` +
      (cardTitle ? sep + `<span class="font-semibold">${escapeHtml(cardTitle)}</span>` : "");
  }

  function bindTitleLiveUpdate() {
    const input = qs("#modal-body input[name='title']");
    if (!input || input.dataset.breadcrumbBound === "1") return;
    input.dataset.breadcrumbBound = "1";
    ["input", "change", "blur"].forEach(evt =>
      input.addEventListener(evt, renderBreadcrumb, { passive: true })
    );
  }

  // corrige alias de tema (base.html chama setModalTheme)
  if (typeof window.setModalTheme !== "function" && typeof window.cardSetTheme === "function") {
    window.setModalTheme = function (mode) { window.cardSetTheme(mode); };
  }

  // wrap de openModal
  if (typeof window.openModal === "function" && !window.openModal.__BREADCRUMB_WRAPPED__) {
    const _open = window.openModal;
    window.openModal = function () {
      const out = _open.apply(this, arguments);
      try { renderBreadcrumb(); bindTitleLiveUpdate(); } catch (_e) {}
      return out;
    };
    window.openModal.__BREADCRUMB_WRAPPED__ = true;
  }

  // eventos HTMX
  ["htmx:afterSwap", "htmx:afterSettle"].forEach(evtName => {
...
// =====================================================
// Breadcrumb do Modal (único e estável)
// - Captura contexto na HOME no clique do board e guarda em sessionStorage
// - Renderiza no modal: [Contexto] -> [Board] -> [Card]
// - Atualiza em: openModal, htmx swap/settle, digitação no title
// =====================================================
(function () {
  ...
  // ---------- Contexto (captura na HOME) ----------
  const CTX_KEY = "boardsContextLabel";

  function isBoardsIndexPage() {
    // Heurística simples: lista de boards normalmente tem cards/links
    return !!document.querySelector(".board-card, .nt-link, [data-board-id], a[href*='/board/']");
  }

  function extractSectionLabelFromEl(el) {
    const section = el.closest("section, .section, .boards-section, div");
    if (!section) return "";
    const h = section.querySelector("h2, h3, .section-title, [data-section-title]");
    return (h && h.textContent ? h.textContent : "").trim();
  }

  function installBoardsIndexContextCaptureOnce() {
    if (document.body.dataset.boardsCtxCaptureBound === "1") return;
    document.body.dataset.boardsCtxCaptureBound = "1";

    // Captura label no clique do board (antes de navegar)
    document.body.addEventListener(
      "click",
      function (ev) {
        ...
        if (!/\/board\/\d+\/?/.test(href)) return;

        // tenta descobrir o "bucket" na home (Meus quadros/Compartilhados/Favoritos…)
        const label =
          extractSectionLabelFromEl(a) ||
          extractSectionLabelFromEl(a.closest(".board-card") || a) ||
          "Meus quadros";

        try {
          sessionStorage.setItem(CTX_KEY, label);
        } catch (_e) {}
      },
      true
    );
  ...
  function getContextLabel() {
    try {
      const v = sessionStorage.getItem(CTX_KEY);
      return (v || "").trim() || "Meus quadros";
    } catch (_e) {
      return "Meus quadros";
    }
  }
  ...
  function ensureBreadcrumbEl() {
    // âncora: tenta achar uma área de “top bar” do modal (CM ou legado)
    const modal = qs("#modal");
    if (!modal) return null;

    const anchor =
      qs("#modal .theme-dot")?.closest("div") ||
      qs("#card-modal-root") ||
      qs("#cm-root") ||
      qs("#modal-body");
  ...
  function renderBreadcrumb() {
    ...
    const boardsUrl = getBoardsIndexUrl();
    const contextLabel = getContextLabel();
    const boardName = getBoardName() || "Quadro";
    const cardTitle = getCardTitle();
    ...
    host.innerHTML =
      ...
      `<span>${escapeHtml(boardName)}</span>` +
      (cardTitle
        ? sep + `<span class="font-semibold">${escapeHtml(cardTitle)}</span>`
        : "");
  }
  ...
  function bindTitleLiveUpdate() {
    ...
    if (input.dataset.breadcrumbBound === "1") return;
    input.dataset.breadcrumbBound = "1";

    input.addEventListener("input", renderBreadcrumb, { passive: true });
    input.addEventListener("change", renderBreadcrumb, { passive: true });
    input.addEventListener("blur", renderBreadcrumb, { passive: true });
  }

  // ---------- Hooks ----------
  // 1) HOME: captura contexto ao clicar no board
  if (isBoardsIndexPage()) installBoardsIndexContextCaptureOnce();

  // 2) wrap openModal (se existir)
  if (typeof window.openModal === "function" && !window.openModal.__BREADCRUMB_WRAPPED__) {
    const _open = window.openModal;
    window.openModal = function () {
      const out = _open.apply(this, arguments);
  ...
  // 3) HTMX: conteúdo do modal trocou
  ["htmx:afterSwap", "htmx:afterSettle"].forEach((evtName) => {
